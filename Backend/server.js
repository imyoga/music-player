...
const broadcastTimerUpdate = () => {
  const update = {
    ...
    timestamp: Date.now(),
    precision: 0.1 // indicates 0.1 second precision
  };
...
const startTimerCountdown = () => {
  ...
  timerState.interval = setInterval(() => {
    if (timerState.remainingTime > 0) {
      timerState.remainingTime--; // Decrement by 1 tenth of a second
      broadcastTimerUpdate();
      // Save state less frequently to reduce I/O - every second (10 tenths)
      if (timerState.remainingTime % 10 === 0) {
        saveTimerState();
      }
    } else {
      // Timer finished
  ...
  }, 100); // 100ms = 0.1 second precision
};
...
app.listen(PORT, () => {
  ...
  console.log(`High-precision timer with 0.1 second (100ms) accuracy`);
  ...
  console.log(`  GET  /api/timer/stream   - Real-time timer updates (SSE, 100ms)`);
});
